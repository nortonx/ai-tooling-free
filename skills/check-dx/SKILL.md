...
Run the tool to collect real firing data. If the binary is not installed locally or globally, fall back to **config-only analysis** and note that limitation in the final output. Discard stderr output according to your shell environment (e.g. redirecting to null) if you wish to suppress noise, but do not hardcode POSIX-only redirections in cross-platform commands.

| Tool | Command |
|---|---|
| eslint | `npx eslint . --format json --no-error-on-unmatched-pattern` |
| prettier | `npx prettier --list-different .` *(prettier rules are options, not granular rules; evaluate each option set in config)* |
| biome | `npx @biomejs/biome lint . --reporter=json` *(if biome linter is disabled, this returns zero firings; that is expected; the formatter-options path still produces evaluable items in Step 4)* |
...
## Step 5: Dispatch 5 evaluations per item

Evaluate each item across 5 independent specialist lenses.

**Multi-runtime execution guidelines:**
- **Claude Code**: Use the `Agent` tool with `subagent_type: general-purpose`. Send all 5 evaluations in a single turn with 5 tool calls for concurrent execution.
- **Antigravity / Gemini CLI**: Use `invoke_subagent` with 5 parallel entries in the `Subagents` array.
- **Single-agent runtimes (such as GitHub Copilot CLI)**: When parallel subagent dispatch is unavailable, evaluate each of the 5 lenses sequentially for each item, returning the JSON object directly into context.

Each evaluation receives:
- The rule/option ID plus its current setting
...
**Evaluation prompts**: each evaluation receives exactly one of these as its task:
...
- If 3 or 4 returned, compute Wilson on `n = <returned>` instead of 5 and append `(partial: <n>/5)` to that item's row in Step 7 output.
...
- Compute Wilson lower bound (n=5, z=1.96):
  ...
  |---|---|---|
  | 5.0 (5xkeep) | 1.0 | approx 0.566 |
  | 4.5 (4xkeep, 1xtune) | 0.9 | approx 0.464 |
  | 4.0 (4xkeep, 1xdrop) | 0.8 | approx 0.376 |
  | 3.5 (3xkeep, 1xtune, 1xdrop) | 0.7 | approx 0.299 |
  | 3.0 (3xkeep, 2xdrop) | 0.6 | approx 0.231 |
  | 2.5 (2xkeep, 1xtune, 2xdrop) | 0.5 | approx 0.187 |
  | 2.0 (2xkeep, 3xdrop) | 0.4 | approx 0.118 |
  | 1.0 (1xkeep, 4xdrop) | 0.2 | approx 0.036 |
  | 0.0 (5xdrop) | 0.0 | approx 0.000 |
...
| **Tune (mixed signals)** | 0.10 to 0.30 |
| **Drop (low value)** | < 0.10 |
...
- (Optional) Note about config-only mode if tool was not installed.
...
- For each "tune" recommendation, the suggested adjustment column must be concrete (an option name plus new value, not vague advice).
- If the tool is not installed and the fallback config-only analysis runs, every Wilson score is necessarily less reliable: say so explicitly in the summary.
