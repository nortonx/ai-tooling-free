...
> ⚠️ **Review the changes first.** Show the working-tree status and the commit message you are about to use, and confirm the changes are what is intended: this stages everything, commits, and pushes to remote. Running `/ship-it` is the user explicit authorization to commit and push for this run (it waives the usual "ask before committing/pushing" rule for this invocation only).
...
(Copilot CLI and skills do not substitute `$ARGUMENTS`; include the branch name or issue id inline in your prompt when you want to override.)
...
3. **Branch**:
   - On the **default branch** (`main`/`master`) -> create and switch to a new branch (`git switch -c <name>`). Name it from the argument if given, else derive `<type>/<slug>`, where `<type>` in `feat|fix|refactor|docs|test|chore|perf|ci`, `<slug>` = short kebab summary of the change. An issue-id argument like `123` -> `<type>/<issue-id>-<slug>`.
...
7. **Report** in 2 to 3 lines: branch name, commit subject, and push result (include any PR-create hint git prints).
