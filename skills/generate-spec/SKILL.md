...
1. **Check for an existing `./specs/` directory.** If it does not exist, tell the user you will create it at write-time and ask whether they want a specific subdirectory (e.g., `frontend/`, `backend/`, `api/`) or a flat layout.
...
2. **List existing specs** by globbing `./specs/**/*.md`. If any exist:
   ...
   - **Heuristic-override path**: if the user prompt also contains `as <scope>` or `--scope=<scope>` (e.g., `/generate-spec User Bulk Import as backend`), use that scope and skip the keyword heuristic.
...
3. **Check for a `./specs/README.md` or `./CLAUDE.md`**: either may document project-specific template overrides. If found, treat its guidance as higher priority than this command default template.

4. After orienting, tell the user in one sentence: how many existing specs were found, which (if any) you will use as a reference, and the detected subdirectory layout.

If no existing specs are present, announce that you will use the canonical 9-section template described in Step 3 and continue.
...
**Q1. Scope / subdirectory.** Propose a target subdirectory based on Step 1 detection (e.g., "Save under `specs/frontend/`?"). If no subdirectories exist in `./specs/`, propose flat placement (`./specs/<slug>.spec.md`). Let the user override.
...
**Q5. Functional requirements.** "List the behaviors this feature must have. I will number them as `FR-1`, `FR-2`, ... `auto` proposes 4-6 starter FRs based on the reference spec and your purpose."
...
**Q8. Acceptance criteria.** "What observable checks prove this works? I will render them as `- [ ]` checkboxes. `auto` derives them directly from your functional requirements."
...
- `## Current Implementation`: always three bolded sub-labels:
  ...
  For a brand-new feature that has not been built yet, use `_(to be filled in during implementation)_` for each.
- `## Open Questions / Future Enhancements`: bullets. Include only if the user provided content.

**Consistency with existing repo:** if Step 1 found local specs, match their exact heading casing, bullet style, and any minor idiomatic differences (e.g., some projects label `FR-N` vs. `FR.N`, or use `Required` instead of `Dependencies`). Your reference spec wins over this template defaults when they conflict.
...
On `yes`: use the environment file writing tool to create the spec at the agreed target path, ensuring parent directories are created if they do not exist.
...
- Never invent file paths, API endpoints, module names, or env var names that are not grounded in user answers or in the reference spec. When uncertain, ask or `skip` rather than guess.
